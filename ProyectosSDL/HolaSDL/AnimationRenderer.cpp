...
AnimationRenderer::AnimationRenderer(Texture* image, Uint32 movementFrames, Uint32 attackMovementFrames, Uint32 totalFrames, Uint32 cooldown, bool character, bool direccionable) : image_(image), movementFrames_(movementFrames), attackMovementFrames_(attackMovementFrames), totalFrames_(totalFrames), cooldown_(cooldown), character(character), direccionable(direccionable)
{
}
...
void AnimationRenderer::render(Entity* o, Uint32 time) {
	...
	if (character) {
		if (o->getVelocity().magnitude() != 0) {
			clip =
				RECT((frame_ + 2) * image_->getWidth() / (totalFrames_),
					dir(o) * image_->getHeight() / movements_,
					image_->getWidth() / (totalFrames_),
			...
			if (o->getComponent<Player>() != nullptr && !attackInProgress_) {
				w = o->getComponent<Player>()->getWeaponId();
				attackFrames_ = initialAttackFrame_ = movementFrames_ + 6 * w;
				attackInProgress_ = true;
			}
			clip =
				RECT((attackFrames_)*image_->getWidth() / (totalFrames_),
			...
			if (time > actualTime_ + cooldown_) {
				if (attackFrames_ < (initialAttackFrame_ + attackMovementFrames_) - 1)
					attackFrames_++;
				else {
					attackFrames_ = 14;
					frame_ = 0;
			...
			clip =
				RECT(idleFrame_* image_->getWidth() / (totalFrames_),
					dirIddle(o) * image_->getHeight() / movements_,
					image_->getWidth() / (totalFrames_),
					image_->getHeight() / movements_);
